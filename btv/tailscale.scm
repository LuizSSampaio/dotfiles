     ...
     (with-imported-modules '((guix build utils))
       #~(begin
           ...
           (if (file-is-directory? #$src)
               (begin
                 (copy-recursively #$src "." #:keep-mtime? #t)
...
(define (tailscale-profile-service config)
  (list tailscale))
  ...
  (service-type
   ...
   (extensions
    (list (service-extension shepherd-root-service-type tailscale-shepherd-service)
          (service-extension profile-service-type tailscale-profile-service)))
